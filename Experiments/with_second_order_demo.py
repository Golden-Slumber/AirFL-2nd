...
from Algorithms.Logistic.Solver.GIANT_logistic_solver import GIANTLogisticSolver
from Algorithms.Logistic.Solver.DANE_logistic_solver import DANELogisticSolver
from Algorithms.Logistic.Solver.ACCADE_logistic_solver import ACCADELogisticSolver
from constants import DCA_ONLY, GS_DCA, PERFECT_AGGREGATION, color_list, marker_list, second_order_list, SDR_ONLY
import sys
...
class WithSecondOrderDemo(Demo):
    def perform_training(self, tau_list, k_list, is_search=True, newton_iter=100):
        for r in range(self.repeat):
            for i in range(len(k_list)):
                for j in range(len(tau_list)):
                    print('repeat ' + str(r) + ' : k = ' + str(k_list[i]) + ' , tau = ' + str(tau_list[j]))

                    # h_mat = numpy.zeros((self.max_iter, k_list[i], self.m))
                    ...
                    err, acc = solver.train(self.gamma, self.w_opt, max_iter=self.max_iter,
                                            is_search=is_search, newton_max_iter=newton_iter)
                    out_file_name = home_dir + 'Outputs/with_second_order_demo/with_second_order_demo_dca only_' + self.data_name + '_antenna_' + str(
                        k_list[i]) + '_tau_' + str(tau_list[j]) + '_repeat_' + str(r) + '.npz'
                    numpy.savez(out_file_name, err=err, acc=acc, data_name=self.data_name)

                    solver = GIANTLogisticSolver(m=self.m, h_mat=h_mat, tau=tau_list[j], p=self.p,
                                                 x_test=self.x_test, y_test=self.y_test, opt_mode=DCA_ONLY)
                    solver.fit(self.x_train, self.y_train, self.data_size_list)
                    err, acc = solver.train(self.gamma, self.w_opt, max_iter=self.max_iter,
                                            is_search=is_search, newton_max_iter=newton_iter)
                    out_file_name = home_dir + 'Outputs/with_second_order_demo/with_second_order_demo_GIANT_' + self.data_name + '_antenna_' + str(
                        k_list[i]) + '_tau_' + str(tau_list[j]) + '_repeat_' + str(r) + '.npz'
                    numpy.savez(out_file_name, err=err, acc=acc, data_name=self.data_name)

                    solver = DANELogisticSolver(m=self.m, h_mat=h_mat, tau=tau_list[j], p=self.p,
                                                x_test=self.x_test, y_test=self.y_test, opt_mode=DCA_ONLY)
                    solver.fit(self.x_train, self.y_train, self.data_size_list)
                    err, acc = solver.train(self.gamma, self.w_opt, max_iter=self.max_iter,
                                            is_search=is_search, newton_max_iter=newton_iter)
                    out_file_name = home_dir + 'Outputs/with_second_order_demo/with_second_order_demo_DANE_' + self.data_name + '_antenna_' + str(
                        k_list[i]) + '_tau_' + str(tau_list[j]) + '_repeat_' + str(r) + '.npz'
                    numpy.savez(out_file_name, err=err, acc=acc, data_name=self.data_name)
    ...
    def plot_results_versus_iteration(self, data_name, k, tau, methods, repeat, max_iter, legends):
        ...
        for i in range(len(methods)-2):
            for r in range(repeat):
                file_name = home_dir + 'Outputs/with_second_order_demo/with_second_order_demo_' + methods[
        ...
        # pre-process
        comm_rounds = numpy.zeros(int(max_iter / 2) + 1)
        for i in range(int(max_iter / 2) + 1):
            comm_rounds[i] = 2 * i
        comm_err_mat = numpy.zeros((len(methods) - 2, repeat, int(max_iter / 2) + 1))
        comm_acc_mat = numpy.zeros((len(methods) - 2, repeat, int(max_iter / 2) + 1))
        for i in range(len(methods) - 2):
            for r in range(repeat):
                comm_err_mat[i][r] = err_mat[i + 2][r][0:int(max_iter / 2) + 1]
                comm_acc_mat[i][r] = acc_mat[i + 2][r][0:int(max_iter / 2) + 1]
        ...
        for i in range(len(methods) - 2):
            line, = plt.semilogy(comm_rounds, numpy.median(comm_err_mat[i], axis=0), color=color_list[i + 2],
                                 linestyle='-',
        ...
        image_name = home_dir + 'Outputs/with_second_order_demo/_with_second_order_demo_err_' + data_name + '_antenna_' + str(
            k) + '_tau_' + str(tau) + '.pdf'
        ...
        for i in range(2):
            ...
            line_list.append(line)
        for i in range(len(methods) - 2):
            line, = plt.semilogy(comm_rounds, numpy.median(comm_acc_mat[i], axis=0), color=color_list[i + 2],
                                 linestyle='-',
                                 marker=marker_list[i + 2],
                                 markerfacecolor='none', ms=7, markeredgewidth=2.5, linewidth=2.5, clip_on=False)
            line_list.append(line)
        ...
        image_name = home_dir + 'Outputs/with_second_order_demo/_with_second_order_demo_acc_' + data_name + '_antenna_' + str(
            k) + '_tau_' + str(tau) + '.pdf'
        fig.savefig(image_name, format='pdf', dpi=1200)
        plt.show()
...
if __name__ == '__main__':
    ...
    tau_list = [1e-5]
    k_list = [5]
    legends = ['Proposed Algorithm', 'Baseline 0', 'Baseline 3', 'Baseline 4']
    ...
    for data_name in datasets:
        ...
        distance_list[0: int(m / 2)] = numpy.random.randint(5, 10, size=int(m / 2))
        distance_list[int(m / 2):] = numpy.random.randint(100, 120, size=int(m / 2))
        ...
        demo = WithSecondOrderDemo(data_name, max_iter, repeat, gamma, sigma, p, m, distance_list, data_size_list)

        demo.fit(x_mat, y_vec)
        demo.perform_training(tau_list, k_list, is_search=is_search, newton_iter=newton_iter)

        second_list = ['ACCADE', 'perfect', 'GIANT', 'DANE']
        for k in k_list:
            for tau in tau_list:
                demo.plot_results_versus_iteration(data_name, k, tau, second_list, repeat, max_iter + 1, legends)
